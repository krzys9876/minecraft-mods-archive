...
import com.krzyskrzys.utils.Utils;
...
public class BitmapFont {
	...
	public BitmapFont(File bmpFile, File metaFile) {
		...
		bmpFileSize=(int)Utils.getLong(fileArr,2);
		bmpOffset=(int)Utils.getLong(fileArr,10);
		bmpWidth=(int)Utils.getLong(fileArr,18);
		bmpHeight=(int)Utils.getLong(fileArr,22);
	...
	public boolean[][] getCharArr(String ch) {
		...
		for(int h=0;h<fontH;h++) {
			for(int w=0;w<fontW;w++) {
				...
				int bit=Utils.getBit(fileArr[bmpOffset+ind/8],7-(ind)%8);
				charArr[w][h]=bit==0;
			}			
		}
		return charArr;
